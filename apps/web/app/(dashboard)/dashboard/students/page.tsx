...
import { Search, MoreHorizontal, Mail, Calendar, TrendingUp, BookOpen, Save } from "lucide-react";
...
const getDefaultObjectives = (subject: string) => {
...
export default function StudentsPage() {
    ...
    useEffect(() => {
        api<{ students: { id: string; name: string; email: string; yearGroup: string | null; subject: string; subjectId: string }[] }>('/tutor/students')
            .then(data => {
                setStudents((data.students || []).map(s => {
                    const objectives = getDefaultObjectives(s.subject);
                    const progress = Math.round(objectives.reduce((a, o) => a + o.value, 0) / objectives.length);
                    return { ...s, objectives, progress, isEditing: false };
                }));
            })
            .catch(() => setStudents([]))
    ...
    const updateObjective = (studentId: string, index: number, newValue: number) => {
        setStudents(prev => prev.map(s => {
            ...
            return { ...s, objectives: newObjectives, progress: Math.round(total / newObjectives.length) };
        }));
    };

    const handleSave = async (student: StudentData) => {
        try {
            // Save each objective as a progress record
            for (const obj of student.objectives) {
                await api('/tutor/progress', {
                    method: 'POST',
                    body: {
    ...
    const filteredStudents = students.filter(student =>
        ...
        student.subject.toLowerCase().includes(debouncedSearch.toLowerCase())
    );
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Button className="btn-secondary">Invite New Student</Button>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredStudents.map((student) => (
                    <div key={student.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col h-full">
                        <div className="p-6 flex items-start justify-between border-b border-slate-50">
                            <div className="flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 font-medium">{student.yearGroup || 'Year group not set'}</p>
                                </div>
                            </div>
                        ...
                        <div className="p-6 space-y-4 flex-1">
                            ...
                            <div className={cn("rounded-xl transition-all duration-300", student.isEditing ? "bg-white p-4 -mx-2 ring-1 ring-slate-200" : "")}>
                                ...
                                {student.isEditing && (
                                    <div className="space-y-4 mt-4 animate-in slide-in-from-top-2">
                                        ...
                                        <div className="flex gap-2 pt-2">
                                            <Button size="sm" className="w-full h-8 bg-emerald-600 hover:bg-emerald-700 text-white" onClick={() => handleSave(student)}>
                ...
                {filteredStudents.length === 0 && !loading && (
                    <div className="col-span-full bg-white rounded-xl border border-dashed border-slate-300 flex flex-col items-center justify-center p-6 text-center min-h-[300px]">
                        <Calendar className="w-12 h-12 text-slate-300 mb-4" />
                        <h3 className="font-bold text-slate-900">No students yet</h3>
                        <p className="text-sm text-slate-500 mt-1">Students will appear here once they book sessions with you.</p>
                    </div>
                )}

                <div className="bg-white rounded-xl border border-dashed border-slate-300 flex flex-col items-center justify-center p-6 text-center hover:bg-slate-100 transition-colors cursor-pointer group min-h-[300px]">
